...
import { loadColumns } from './configureMembershipsCols';
...
const TableSampleMemberships = ({
  ...
  useEffect(() => {
    if (membershipsNotify.showNotification) {
      notify(
        membershipsNotify.typeNotification,
        membershipsNotify.textNotification,
      );
    }
  }, [membershipsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/memberships/memberships-view/?id=${id}`);
  };
...
export default TableSampleMemberships;
