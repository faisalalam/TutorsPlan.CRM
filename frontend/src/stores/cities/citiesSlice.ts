...
export const deleteItemsByIds = createAsyncThunk(
  ...
  async (data: any, { rejectWithValue }) => {
    try {
      await axios.post('cities/deleteByIds', { data });
    } catch (error) {
      if (!error.response) {
        throw error;
      }
...
export const uploadCsv = createAsyncThunk(
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const result = await axios.post('cities/bulk-import', data, {
        headers: {
...
export const update = createAsyncThunk(
  ...
  async (payload: any, { rejectWithValue }) => {
    try {
      const result = await axios.put(`cities/${payload.id}`, {
        id: payload.id,
        data: payload.data,
      });
      return result.data;
...
export const citiesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteItemsByIds.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Cities has been deleted');
    });
    ...
    builder.addCase(uploadCsv.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Cities has been uploaded');
    });
...
export const { setRefetch } = citiesSlice.actions;
